...
class ExpenseController extends GetxController {
  ...
  void onInit() {
    ...
    // Add listener to dashboard controller's groups
    final dashboardController = Get.find<DashboardController>();
    ever(dashboardController.groups, (_) {
      fetchGroups();
    });

    // Listen to dashboard controller updates
    ever(dashboardController.userProfile, (_) {
      print("Profile updated in Dashboard, refreshing expense data");
      refreshExpenseData();
    });
  }
  ...
  void refreshExpenseData() {
    ...
    if (selectedGroup.value != null) {
      ...
      final freshGroup = ExpenseManagerService.getGroupById(selectedGroup.value!.id!);
      if (freshGroup != null) {
        print("Found fresh group: ${freshGroup.groupName}");
        print("Fresh members: ${freshGroup.members.map((m) => '${m.name}:${m.phone}')}");

        // Update the group and members
        selectedGroup.value = freshGroup;
        members.assignAll(freshGroup.members);

        // Update selected payer if it exists
        if (selectedPayer.value != null) {
          final updatedPayer = freshGroup.members.firstWhereOrNull(
                  (m) => m.phone == selectedPayer.value!.phone
          );
          if (updatedPayer != null) {
            selectedPayer.value = updatedPayer;
            print("Updated payer: ${updatedPayer.name}");
          }
        }

        // Update selected members
        final updatedSelectedMembers = selectedMembers.map((member) {
          return freshGroup.members.firstWhereOrNull(
                  (m) => m.phone == member.phone
          ) ?? member;
        }).toList();
        selectedMembers.assignAll(updatedSelectedMembers);
  ...
  void refreshMemberData() {
    if (selectedGroup.value != null) {
      ...
      final updatedGroup = ExpenseManagerService.getGroupById(selectedGroup.value!.id!);
      if (updatedGroup != null) {
        selectedGroup.value = updatedGroup;
        members.value = updatedGroup.members.toList();

        // Update selected payer if it was the updated profile
        if (selectedPayer.value != null) {
          final updatedMember = updatedGroup.members.firstWhereOrNull(
                  (m) => m.phone == selectedPayer.value!.phone
          );
          if (updatedMember != null) {
            selectedPayer.value = updatedMember;
          }
        }

        // Update selected members
        final updatedSelectedMembers = selectedMembers.map((member) {
          return updatedGroup.members.firstWhereOrNull(
                  (m) => m.phone == member.phone
          ) ?? member;
        }).toList();
        selectedMembers.value = updatedSelectedMembers;
      }
    }
  }
  ...
  void initializeExpenseData(Expense? expense) {
    print("Initializing expense data");
    _resetData();
    ...
    if (expense != null) {
      ...
      if (expense.group != null) {
        // Get fresh group data
        final freshGroup = ExpenseManagerService.getGroupById(expense.group!.id!);
        if (freshGroup != null) {
          selectedGroup.value = freshGroup;
          members.assignAll(freshGroup.members);
          print("Initialized members: ${members.map((m) => '${m.name}:${m.phone}')}");
        }
      }

      // Set payer with fresh data
      if (expense.group != null) {
        final freshPayer = members.firstWhereOrNull(
                (m) => m.phone == expense.paidByMember.phone
        );
        selectedPayer.value = freshPayer ?? expense.paidByMember;
      } else {
      ...
      // Set selected members with fresh data
      final freshSelectedMembers = expense.splits.map((split) {
        return members.firstWhereOrNull(
                (m) => m.phone == split.member.phone
        ) ?? split.member;
      }).toList();
      selectedMembers.assignAll(freshSelectedMembers);

      if (expense.divisionMethod == DivisionMethod.unequal) {
        _memberAmountControllers.clear();
        for (var split in expense.splits) {
          _memberAmountControllers[split.member.phone] =
              TextEditingController(text: split.amount.toStringAsFixed(2));
        }

        for (var member in members) {
          if (!_memberAmountControllers.containsKey(member.phone)) {
            _memberAmountControllers[member.phone] =
  ...
  void onClose() {
    ...
    _memberAmountControllers.forEach((_, controller) => controller.clear());
    _memberAmountControllers.clear();
    super.onClose();
  }
...
// Optional: Helper method to initialize when editing an expense
  ...
  void fetchGroups() {
    ...
    groups.assignAll(fetchedGroups); // Use assignAll instead of value =
