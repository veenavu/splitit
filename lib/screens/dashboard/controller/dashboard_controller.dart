...
class DashboardController extends GetxController {
  ...
  void onInit() {
    super.onInit();
    ever(userProfile, (_) => _onProfileUpdate());
    getProfile().then((_) {
      loadGroups();
      getBalanceText();
    });
  ...
  void updateUserProfile(Profile updatedProfile) {
    ...
    getBalanceText();
    update(['dashboard_profile']); // Force UI update for specific widgets
  }
  ...
  void _onProfileUpdate() {
    if (userProfile.value != null) {
      getBalanceText();
      update(['dashboard_profile']); // Using unique ID for update
    }
  }
  ...
  Future<void> getProfile() async {
    ...
    final updatedProfile = ExpenseManagerService.getProfileByPhone(phone) ??
        Profile(id: 0, name: "User", email: "noob", phone: "2173123");

    userProfile.value = updatedProfile;
    update(['dashboard_profile']); // Update UI when profile is fetched
  }
  ...
  void getBalanceText() {
    balanceText.value = userProfile.value != null
        ? ExpenseManagerService.getMemberBalanceText(
      Member(
        id: userProfile.value!.getProfileId(),
        name: userProfile.value!.name,
        phone: userProfile.value!.phone,
  ...
  String getGroupBalanceText(Group group) {
    if (userProfile.value == null) return "Loading...";
    ...
    final balance = ExpenseManagerService.calculateGroupBalance(group, member);
