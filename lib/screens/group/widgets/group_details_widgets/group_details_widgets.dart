...
class ExpenseListItem extends StatelessWidget {
  ...
  String _getExpenseStatusText(bool isYou) {
    if (expense.status == ExpenseStatus.fullySettled) {
      return 'Settled up';
    }

    double netAmount = calculateNetAmount(expense, phoneNumber);
    if (netAmount == 0) return 'Settled up';

    return isYou ? 'You lent' : 'You borrowed';
  }

  bool get isSettlementExpense => expense.description.toLowerCase().contains('settlement');
  ...
  Widget _buildSettlementCard() {
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Text(
          '${expense.paidByMember.name} paid ₹${expense.totalAmount.toStringAsFixed(2)} to ${expense.splits.first.member.name}',
          style: TextStyle(
  ...
  Widget _buildRegularExpenseCard() {
    bool isYou = phoneNumber == expense.paidByMember.phone;
    double netAmount = calculateNetAmount(expense, phoneNumber);
    bool isSettled = expense.status == ExpenseStatus.fullySettled || netAmount == 0;
    ...
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Text(
          '${isYou ? "You" : expense.paidByMember.name} paid ₹${expense.totalAmount.toStringAsFixed(2)}',
          style: TextStyle(
            color: Colors.grey.shade600,
            fontSize: 14,
          ),
  ...
  @override
  Widget build(BuildContext context) {
    return isSettlementExpense ? _buildSettlementCard() : _buildRegularExpenseCard();
  }
}
