...
class ExpenseManagerService {
 ...
 // ___________________________________________________________________________________
  ...
  static List<Expense> getExpensesByDateRange(DateTime start, DateTime end) {
    ...
    return expenses
        .where((expense) =>
    expense.createdAt.isAfter(start) && expense.createdAt.isBefore(end))
        .toList();
  }
  ...
  static Map<DateTime, double> getMonthlyExpenses(Group? group) {
    ...
    for (var expense in expenses) {
      ...
      monthlyExpenses[monthYear] =
          (monthlyExpenses[monthYear] ?? 0) + expense.totalAmount;
    }
  ...
  static List<SettlementTransaction> getSimplifiedSettlements(Group group) {
    ...
    for (var expense in expenses) {
      balances[expense.paidByMember] =
          (balances[expense.paidByMember] ?? 0) + expense.totalAmount;

      for (var split in expense.splits) {
        balances[split.member] =
            (balances[split.member] ?? 0) - split.amount;
      }
    }

    List<SettlementTransaction> settlements = [];
    var sortedMembers = balances.entries.toList()
  ...
  static GroupStatistics getGroupStatistics(Group group) {
    ...
    return GroupStatistics(
      ...
      averageExpenseAmount: expenses.isEmpty
          ? 0
          : expenses.fold(0.0, (sum, exp) => sum + exp.totalAmount) / expenses.length,
      monthlyExpenses: monthlyExpenses,
      categoryExpenses: categoryExpenses,
      memberStatistics: memberStats,
      expenseCount: expenses.length,
      activeMembers: group.members.length,
      mostActiveCategory: categoryExpenses.entries
          .reduce((a, b) => a.value > b.value ? a : b)
          .key,
      mostActivePayer: memberStats.entries
          .reduce((a, b) =>
      a.value.totalPaid > b.value.totalPaid ? a : b)
          .key,
    );
  }
}
