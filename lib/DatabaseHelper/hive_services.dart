...
class ExpenseManagerService {
  ...
  static Future<void> recalculateAllBalances() async {
    ...
    final expensesBox = Hive.box<Expense>(ExpenseManagerService.expenseBoxName);
    ...
    for (var expense in expensesBox.values) {
      if (expense.status != ExpenseStatus.fullySettled) {
          ...
          if (split.member.phone != expense.paidByMember.phone &&
            ...
            if (member != null) {
              ...
              if (expense.group != null) {
                final groupId = expense.group!.id.toString();
                member.balancesByGroup[groupId] =
                    (member.balancesByGroup[groupId] ?? 0.0) + split.amount;
              }
              await member.save();
            }
          }
    ...
    // Process settlements
    for (var settlement in settlementBox.values) {
      final payer = membersBox.get(settlement.payer.phone);
      final receiver = membersBox.get(settlement.receiver.phone);

      // Update relevant group balances based on the expenses in the settlement
      for (var expenseSettlement in settlement.expenseSettlements) {
        final expense = expenseSettlement.expense;
        if (expense.group != null) {
          final groupId = expense.group!.id.toString();
          if (payer != null) {
            payer.balancesByGroup[groupId] =
                (payer.balancesByGroup[groupId] ?? 0.0) - expenseSettlement.settledAmount;
          }
          if (receiver != null) {
            receiver.balancesByGroup[groupId] =
                (receiver.balancesByGroup[groupId] ?? 0.0) + expenseSettlement.settledAmount;
          }
        }
      }

      if (payer != null) await payer.save();
      if (receiver != null) await receiver.save();
    }
  }
  ...
  //   await profile.save();
  // }
  static Future<void> updateProfile(Profile updatedProfile) async {
    try {
      final box = Hive.box<Profile>(profileBoxName);

      // Find the index of the existing profile
      int? index;
      for (int i = 0; i < box.length; i++) {
        final profile = box.getAt(i);
        if (profile?.phone == updatedProfile.phone) {
          index = i;
          break;
        }
      }
...
// Helper function to get amount owed/to be received for a group
  ...
  static String getMemberBalanceText(Member member) {
    final allGroups = getAllGroups();
  ...
  static String getAmountBalanceText(double amount) {
    if (amount > 0) {
      return 'gets back ₹${amount.abs().toStringAsFixed(2)}';
    } else if (amount < 0) {
      return 'owes ₹${amount.abs().toStringAsFixed(2)}';
    }
    return 'settled up';
  }
  ...
  static Future<void> recordSettlement({
    ...
    required Member receiver,
    required double amount,
    required List<Expense> settlementExpenses,
  }) async {
    try {
      // Start a transaction
      final expensesBox = Hive.box<Expense>(expenseBoxName);
      ...
      // Calculate how much to settle for each expense
      List<ExpenseSettlement> expenseSettlements = [];
      double remainingAmount = amount;

      for (var expense in settlementExpenses) {
        if (remainingAmount <= 0) break;

        // Find how much payer owes in this expense
        final payerSplit = expense.splits.firstWhereOrNull(
              (split) => split.member.phone == payer.phone,
        );

        if (payerSplit == null || payerSplit.amount <= 0) continue;

        // Calculate settlement amount for this expense
        double settleAmount = payerSplit.amount;
        if (settleAmount > remainingAmount) {
          settleAmount = remainingAmount;
        }
      ...
      if (!settlement.validateSettlement()) {
        throw Exception('Invalid settlement configuration');
      }

      // Update expense states
      for (var expenseSettlement in expenseSettlements) {
        final expense = expenseSettlement.expense;
        final settleAmount = expenseSettlement.settledAmount;

        // Update the split amount
        final split = expense.splits.firstWhere(
              (s) => s.member.phone == payer.phone,
        );
        split.amount -= settleAmount;

        // Update expense status
        if (expense.splits.every((s) => s.amount <= 0.01)) {
      ...
      // Save settlement
      await settlementsBox.add(settlement);
  ...
  static GroupBalance calculateGroupBalance(Group group, Member currentMember) {
    ...
    for (var expense in expenses) {
      ...
      if (expense.paidByMember.phone == currentMember.phone) {
        // Current member paid, calculate remaining amounts from splits
        for (var split in expense.splits) {
          if (split.member.phone != currentMember.phone && split.amount > 0) {
            amountToReceive += split.amount;
          }
        }
      } else {
        // Someone else paid, find current member's remaining split
        final memberSplit = expense.splits.firstWhere(
              (split) => split.member.phone == currentMember.phone,
          orElse: () => ExpenseSplit(member: currentMember, amount: 0),
        );
        if (memberSplit.amount > 0) {
          amountToPayBack += memberSplit.amount;
        }
      }
    }
    ...
    return GroupBalance(
      ...
      netAmount: amountToReceive - amountToPayBack,
    );
  }
}


void logSettlementDebug(Settlement settlement) {
  print('=== Settlement Debug Log ===');
  print('Payer: ${settlement.payer.name}');
  print('Receiver: ${settlement.receiver.name}');
  print('Amount: ${settlement.amount}');
  print('Expense Settlements:');
  for (var es in settlement.expenseSettlements) {
    print('  Expense: ${es.expense.description}');
    print('  Amount: ${es.settledAmount}');
    print('  Status: ${es.expense.status}');
    print('  Remaining splits:');
    for (var split in es.expense.splits) {
      print('    ${split.member.name}: ${split.amount}');
    }
  }
  print('========================');
}
